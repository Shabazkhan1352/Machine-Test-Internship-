  ...
  useEffect(() => {
    const fetchAgents = async () => {
      try {
        ...
        alert('Failed to load subagents');
      }
    };
    fetchAgents();
  }, []);
  ...
  return (
    <div className="p-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
      {agents.map((agent, index) => (
        ...
        >
          <h3 className="text-lg font-semibold text-blue-700 mb-2">Sub Agent</h3>
          ...
          <button onClick={() => navigate(`/subagent/${agent._id}`)} className='bg-black text-white px-4 py-2 mt-3 rounded-md cursor-pointer'>View Assigned Tasks</button>
